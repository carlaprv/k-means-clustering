...
def main():
	...
	#Inserir o endereço de entrada, ou seja, onde estao os arquivos:
	path_in = "../bbc/"
	#Inserir o endereço de saida, onde devem ficar registradas as representacoes:
	path_out = "../corpora/"
	...
	#write_corpus_status = write_corpus(path_out + "corpus_bbc.txt", corpus_list_text)
	...
	write_corpus_status = write_corpus(path_out + "corpus_processado_bbc.txt", pcorpus)
	print("Status da escrita: " + str(write_corpus_status))
	...
	print("Iniciando escrita da representação binária")
	write_bin_status = write_rep(df_binary, path_out + 'rep_binaria_bbc')
	print("Status da escrita: " + str(write_bin_status))

	print("Iniciando escrita da representação tf")
	tf_matrix = tf(pcorpus, term_list)
	write_tf_matrix_status = write_corpus(path_out + "tf_bbc.txt", tf_matrix)
	print("Status da escrita: " + str(write_tf_matrix_status))
	
	print("Iniciando escrita da representação tf_idf")
	tf_idf_matrix = tf_idf(pcorpus, term_list, binary_matrix)
	write_tf_idf_matrix_status = write_corpus(path_out + "tf_idf_bbc.txt", tf_matrix)
	print("Status da escrita: " + str(write_tf_matrix_status))
